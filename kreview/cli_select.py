...
"""CLI command: ``kreview select`` — Feature scoring and hybrid-union selection.
...
hybrid-union selection (top N% AUC ∪ top N% MI) with a variance guard.
...
def select(
    matrices_dir: Path = typer.Option(
        ..., "--matrices-dir", help="Directory with *_matrix.parquet files from kreview extract"
    ),
    top_percentile: float = typer.Option(
        50.0,
        "--top-percentile",
        help="Top X%% of features per metric for hybrid union selection",
    ),
    ...
    """Score features and apply hybrid-union selection to extracted matrices.
    ...
    top features using a hybrid union strategy (top N%% by AUC ∪ top N%%
    by MI).
