...
def run(
    ...
    t0 = time.time()
    paths = Paths(str(cancer_samplesheet), str(healthy_xs1_samplesheet),
                  str(healthy_xs2_samplesheet), str(cbioportal_dir))
    config = LabelConfig(min_vaf=min_vaf, min_variants=min_variants)
    labeler = CtDNALabeler(paths, config)
    ...
    for e in target_evals:
        ...
        if len(model_df) >= 20 and len(np.unique(y)) == 2:
            ...
            if top_feats:
                ...
                model_res, lr_model, rf_model, xgb_model = single_feature_model(X, y, cancer_types=c_types)
