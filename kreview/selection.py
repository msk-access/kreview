...
POSITIVE_LABELS = frozenset(["True ctDNA+", "Possible ctDNA+"])

# Backward-compat aliases (used internally, kept for any external references)
_MODEL_LABELS = MODEL_LABELS
_POSITIVE_LABELS = POSITIVE_LABELS
...
def build_binary_target(
    ...
    """
    mask = df[label_col].isin(MODEL_LABELS)
    model_df = df[mask].copy()
    y = model_df[label_col].isin(POSITIVE_LABELS).astype(int).values
...
def score_features(
    ...
    *,
    ...
    # ── Filter to train-only split to prevent data leakage ──
    # Feature scoring (univariate AUC, MI) must not see holdout data.
    # If no 'split' column exists (pre-v0.0.16 data), use all samples.
    if "split" in matrix_df.columns:
        train_mask = matrix_df["split"] == "train"
        n_excluded = int((~train_mask).sum())
        if n_excluded > 0:
            log.info(
                "score_features_train_only",
                n_total=len(matrix_df),
                n_train=int(train_mask.sum()),
                n_excluded=n_excluded,
            )
            scoring_df = matrix_df[train_mask].copy()
        else:
            scoring_df = matrix_df
    else:
        scoring_df = matrix_df
    ...
    for col in numeric_cols:
        res = evaluate_feature(
            scoring_df[col],
            scoring_df[label_col],
            total_fragments=scoring_df.get("n_total_somatic_snvs"),
            max_vaf=scoring_df.get("max_vaf"),
        )
    ...
    model_df, y = build_binary_target(scoring_df, label_col)
...
def select_features(
    ...
    *,
    ...
    if strategy == "mrmr":
        ...
        # Use train-only samples for mRMR scoring (prevent holdout leakage)
        if "split" in matrix_df.columns:
            scoring_df = matrix_df[matrix_df["split"] == "train"]
            log.info(
                "select_features_train_filter",
                total=len(matrix_df),
                train=len(scoring_df),
                strategy="mrmr",
            )
        else:
            scoring_df = matrix_df  # Backward compatible
    ...
    # Filter to train-only for variance check (prevent holdout leakage)
    if "split" in matrix_df.columns:
        var_check_df = matrix_df[matrix_df["split"] == "train"]
    else:
        var_check_df = matrix_df
    model_df, _ = build_binary_target(var_check_df, label_col)
