...
### Fixing Accidental Python Edits (`nbdev-update`)
If you broke the Golden Rule and edited `kreview/core.py` directly, do NOT panic. 

You can run `nbdev-update` from the command line. This acts "in reverse", reading the source code from the raw `.py` module and aggressively injecting it BACK over the standard cells inside the corresponding Jupyter Notebook in `nbs/`!
