...
cells.append(nbf.v4.new_code_cell("#| default_exp core"))
...
# Cell 5: DuckDB connect and directory scanning
cells.append(nbf.v4.new_code_cell("""#| export
...
# Cell 6: DuckDB Feature Loading Cohort
cells.append(nbf.v4.new_code_cell("""#| export
def load_feature_cohort(
    ...
    \"\"\"Load one feature type across ALL available samples in one DuckDB query.\"\"\"
    ...
    glob_pattern = f"{results_dir}/*/*{feature_suffix}"
    ...
    try:
        if sample_ids:
            # We enforce filtering via DuckDB
            sample_ids_list = list(sample_ids)
            conn.execute("CREATE OR REPLACE TEMP TABLE filter_ids AS SELECT * FROM (SELECT unnest(?::VARCHAR[]) AS sample_id)", [sample_ids_list])
            query = f\"\"\"
                SELECT sub.* FROM ({query}) sub
                INNER JOIN filter_ids f ON sub.sample_id = f.sample_id
            \"\"\"
        ...
        if df.empty:
            log.warning("empty_result", feature=feature_suffix, reason="no matches found", glob_pattern=glob_pattern)
        else:
        ...
        log.error("feature_cohort_load_failed", feature=feature_suffix, error=str(e), glob_pattern=glob_pattern)
        raise
...
def load_metadata_cohort(
    ...
    \"\"\"Load metadata.parquet across all samples for QC and confounding analysis.\"\"\"
    return load_feature_cohort(
        feature_suffix=".metadata.parquet",
        results_dir=results_dir,
        sample_ids=sample_ids,
    )
"""))

# Cell 7: Single Feature / Metadata utilities
cells.append(nbf.v4.new_code_cell("""#| export
...
nb['cells'] = cells
with open('nbs/00_core.ipynb', 'w') as f:
    nbf.write(nb, f)
