...
lb = pd.read_parquet(data_dir / "labels" / "labels.parquet", columns=["SAMPLE_ID", "PATIENT_ID"])
pat = sm[["_sample_id"]].merge(lb, left_on="_sample_id", right_on="SAMPLE_ID", how="left")
...
sels = {s: json.loads((vdir / s / "full_train.json").read_text())["selected"]
        for s in ("boruta_shap", "grootcv")}
a, b = set(sels["boruta_shap"]), set(sels["grootcv"])
print(f"full-train sets: boruta k={len(a)} grootcv k={len(b)} overlap={len(a&b)} jaccard={len(a&b)/len(a|b):.2f}")
print("only boruta:", sorted(a - b)); print("only grootcv:", sorted(b - a))

MODELS = {
    "lr": lambda: Pipeline([("s", StandardScaler()), ("m", LogisticRegression(max_iter=1000, random_state=42))]),
    "rf": lambda: RandomForestClassifier(n_estimators=200, max_depth=6, random_state=42, n_jobs=-1),
    "xgb": lambda: XGBClassifier(n_estimators=100, max_depth=5, eval_metric="logloss", random_state=42, verbosity=0, n_jobs=-1),
}
cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
probs = {s: {m: cross_val_predict(mk(), sm[sels[s]].to_numpy(), y, cv=cv, method="predict_proba")[:, 1]
             for m, mk in MODELS.items()} for s in sels}
...
for m in MODELS:
    ...
    print(f"{m:4s} {np.mean(d_auc):+8.4f} [{qa[0]:+.4f},{qa[1]:+.4f}] {np.mean(d_sens):+10.4f} [{qs[0]:+.4f},{qs[1]:+.4f}]")
