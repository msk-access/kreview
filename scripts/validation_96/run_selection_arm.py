...
def run_draw(X, y, idx, strategy: str, out_path: Path) -> dict:
    ...
    _, selected = _select_multimodal_features(Xd, yd, strategy=strategy, random_state=42)
...
def main() -> None:
    ...
    ap.add_argument("--strategy", choices=["boruta_shap", "grootcv", "leshy", "mi"], required=True)
    ap.add_argument("--draws", default="all", help='"all", or comma list like "0,1,2"')
    ap.add_argument("--full-train", action="store_true", help="also run on the full train split (for phase 2)")
    args = ap.parse_args()
    ...
    assert len(X) == spec["n_rows"], f"matrix rows {len(X)} != indices spec {spec['n_rows']}"

    todo = sorted(spec["draws"], key=int) if args.draws == "all" else args.draws.split(",")
    for b in todo:
        ...
        print(f"[{args.strategy}] draw {b}: {rec['n_selected']} features in {rec['wall_seconds']}s", flush=True)

    if args.full_train:
        out_path = out_dir / "full_train.json"
        if not out_path.exists():
            rec = run_draw(X, y, np.arange(len(X)), args.strategy, out_path)
            print(f"[{args.strategy}] full train: {rec['n_selected']} features in {rec['wall_seconds']}s", flush=True)
